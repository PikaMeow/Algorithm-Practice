    ...
    for(int i=1;i<=m;i++) {
        ...
        for(int j=1;j<=n;j++) {
            ...
            while(tmp!=a[j]) {
                ...
                if(!s[i][index]) {s[i][index]=j;tmp++;}//表示第i条染色体的第index个位置上正在对工件j进行加工
            }
        }
    }
}
...
void cal_time(struct det p[][MAXN],int a[],int n,int s[][MAXN],int len,int pos,int total[]) {
    //n为工件个数，len为总的工序数目，pos为第几条染色体
        ...
        for(int i=1;i<=len;i++) {
           ...
           if(seq==a[part]) if(total[pos]<pt[part][seq]) total[pos]=pt[part][seq];//加工完整个过程的时间
...
void duplicate(int n,int s[][MAXN],int len,int tot,int m,int total[]) {
    ...
    int posi=0;
    for(int i=1;i<=tot;i++) prob[i]=suit[i]/sum;
    for(int i=1;i<=tot;i++) {if(prob[i]>prob[i-1]) posi=i;}
    chos[posi]=1;
    for(int i=1;i<=tot;i++) {prob[i]+=prob[i-1];}
    //printa(prob,tot);
    srand((int)time(0)+rand());
...
void mutate(int s[][MAXN],int len,int pos,int &tot) {
    ...
    if(ran1>ran2) {int t=ran1;ran1=ran2;ran2=t;}//选取中间的某一段，倒置过来
...
void evaluate(int s[][MAXN],int num,int len,int total[],double suit[]) {
    ...
    //printa(suit,num);
    double avg=0;
    for(int i=1;i<=num;i++) avg+=total[i];
    avg=avg*1.0/num;
    printf("%.4f\n",avg);
    /*
...
int main() {
    ...
    int n,len=0,num;
    fscanf(fp,"%d",&n);//工件的个数
    for(int i=1;i<=n;i++) {fscanf(fp,"%d",&a[i]); len+=a[i];}//a[i]为每个工件的工序数，len为总的工序数目
    for(int i=1;i<=n;i++) {
        for(int j=1;j<=a[i];j++) {
            int _m,_t;
            fscanf(fp,"%d%d",&_m,&_t);
    ...
    */
    ...
    double avg_suit=0;
    for(int i=1;i<=num;i++) avg_suit+=suit[i];
    avg_suit/=num;
